...
static bool getNodesLinks(cJSON *obj, cJSON **nodes, cJSON **links)
{
  *nodes = NULL;
  *links = NULL;
...
static cJSON *json_read(const char *path)
{
  size_t data_size = 0;
...
bool json_set(const char *path, char *node_ids, char *link_ids, const char *key, const char *value)
{
  ...
  if (!path || !node_ids || !link_ids || !key || !value) {
    fprintf(stderr, "json: Missing query arguments\n");
    return false;
  }
